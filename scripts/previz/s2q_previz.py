# S2Q previz — THE MADAME ARRIVES. She comes in after Valder has finished
...
madame = spawn_char(col, "PARROT",      ( 0.35,  19.00), h=1.72, prefix="s2q")
...
for name, x, y, h in LEFT + RIGHT:
    standing.append(spawn_char(col, name, (x, y), h=h, prefix="s2q"))
# Dupe far back on the left with his cart, still not part of anything
dupe = spawn_char(col, "DUPE", (-4.60, -5.40), h=1.80, prefix="s2q")
build_cart(col, (-5.20, -6.10), prefix="s2q", with_painting=True)
cart = bpy.data.objects.get("s2q_cart_base")
if cart:
    for o in bpy.data.objects:
        if o.name.startswith("s2q_cart_") and o is not cart:
            o.parent = cart; o.matrix_parent_inverse = cart.matrix_world.inverted()
    box("s2q_painting", (-5.20, -5.70, 1.35), (0.80, 0.05, 0.72), (0.45, 0.45, 0.45, 1)).parent = cart
...
cam_d = bpy.data.cameras.new("CAM_S2Q"); cam_d.lens = 50
cam = bpy.data.objects.new("CAM_S2Q", cam_d); sc.collection.objects.link(cam); sc.camera = cam
cam.location = (0.20, -8.20, 1.58)
cam.rotation_euler = (R(90), 0, 0)              # looking up the hall along +y
for who, ob in ([("MADAME", madame), ("DUPE", dupe)] +
                [(n, o) for (n, _, _, _), o in zip(LEFT + RIGHT, standing)]):
    tag_label(col, who, ob, cam, prefix="s2q")
...
sc.render.filepath = r"C:\Users\UsEr\Downloads\S2Q-Render.MP4"
bpy.ops.wm.save_as_mainfile(filepath=r"C:\Users\UsEr\Downloads\S2Q_previz.blend")
sc.frame_set(1)

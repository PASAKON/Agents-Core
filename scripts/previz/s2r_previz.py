# S2R previz — THE BIDDING BATTLE. The CEO's staging is specific and it is what
...
gent   = spawn_char(col, "GENTLEMAN",  (-2.10, -3.00), h=1.79, prefix="s2r")
bguard = spawn_char(col, "BODYGUARD",  (-3.10, -2.10), h=1.94, prefix="s2r")
madame = spawn_char(col, "PARROT",     ( 2.15, -3.20), h=1.72, prefix="s2r")
valder = spawn_char(col, "VALDER",     ( 0.00, -1.55), h=1.86, prefix="s2r")
regis  = spawn_char(col, "REGISTRAR",  (-3.40, -5.60), h=1.80, prefix="s2r")
...
for name, x, y, h in crowd:
    spawn_char(col, name, (x, y), h=h, prefix="s2r")
dupe = spawn_char(col, "DUPE", (-4.30, 1.60), h=1.80, prefix="s2r")
build_cart(col, (-4.95, 2.30), prefix="s2r", with_painting=True)
cart = bpy.data.objects.get("s2r_cart_base")
if cart:
    for o in bpy.data.objects:
        if o.name.startswith("s2r_cart_") and o is not cart:
            o.parent = cart; o.matrix_parent_inverse = cart.matrix_world.inverted()
    box("s2r_painting", (-4.95, 2.70, 1.35), (0.80, 0.05, 0.72), (0.45, 0.45, 0.45, 1)).parent = cart

cam_d = bpy.data.cameras.new("CAM_S2R"); cam_d.lens = 30
cam = bpy.data.objects.new("CAM_S2R", cam_d); sc.collection.objects.link(cam); sc.camera = cam
cam.location = (0.00, -11.40, 1.66)
cam.rotation_euler = (R(88), 0, 0)              # 2 deg down, otherwise level
for who, ob in (("GENTLEMAN", gent), ("MADAME", madame), ("VALDER", valder),
                ("REGISTRAR", regis), ("BODYGUARD", bguard), ("DUPE", dupe)):
    tag_label(col, who, ob, cam, prefix="s2r")
...
for name, x, y, _ in crowd:
    ob = bpy.data.objects.get(f"s2r_{name}")
    if ob:
        key(ob, 1, x, y); key(ob, 480, x, y)
...
sc.render.filepath = r"C:\Users\UsEr\Downloads\S2R-Render.MP4"
bpy.ops.wm.save_as_mainfile(filepath=r"C:\Users\UsEr\Downloads\S2R_previz.blend")
sc.frame_set(1)

# S2NB previz — THE REVERSE. Same scene, same marks, camera moved OUT of
...
woman   = spawn_char(col, "COLLECTOR_A",     (-1.60, -19.95), h=1.68, prefix="s2nb")
student = spawn_char(col, "STUDENT", ( -0.45, -19.72), h=1.70, prefix="s2nb")
wifeB   = spawn_char(col, "VISITOR_B", ( 0.40, -19.88), h=1.66, prefix="s2nb")
manA    = spawn_char(col, "VISITOR_A", ( 1.05, -19.55), h=1.78, prefix="s2nb")
critic  = spawn_char(col, "CRITIC",  ( 1.80, -19.95), h=1.64, prefix="s2nb")
dupe    = spawn_char(col, "DUPE",      (-2.35, -17.20), h=1.80, prefix="s2nb")
regis   = spawn_char(col, "REGISTRAR", (-3.20, -19.00), h=1.80, prefix="s2nb")
gent    = spawn_char(col, "GENTLEMAN", ( 5.60,  -1.60), h=1.79, prefix="s2nb")
guard   = spawn_char(col, "BODYGUARD", ( 5.95,  -0.90), h=1.94, prefix="s2nb")
build_cart(col, (-3.30, -16.60), prefix="s2nb", with_painting=True)
cart = bpy.data.objects.get("s2nb_cart_base")
if cart:
    cart.rotation_euler.z = R(90)
    for o in bpy.data.objects:
        if o.name.startswith("s2nb_cart_") and o is not cart:
            o.parent = cart; o.matrix_parent_inverse = cart.matrix_world.inverted()
    box("s2nb_painting", (-3.30, -16.20, 1.35), (0.80, 0.05, 0.72), (0.45, 0.45, 0.45, 1)).parent = cart
...
door = box("s2nb_sidedoor", (5.90, -2.00, 1.35), (0.10, 1.30, 2.70), (0.32, 0.10, 0.10, 1))
door.location = (5.90, -2.65, 1.35)
piv = bpy.data.objects.new("s2nb_door_piv", None); sc.collection.objects.link(piv)
...
cam_d = bpy.data.cameras.new("CAM_S2NB"); cam_d.lens = 35
cam = bpy.data.objects.new("CAM_S2NB", cam_d); sc.collection.objects.link(cam); sc.camera = cam
...
for who, ob in (("WOMAN", woman), ("STUDENT", student), ("WIFE", wifeB),
    ...
    tag_label(col, who, ob, cam, prefix="s2nb")
...
sc.render.filepath = r"C:\Users\UsEr\Downloads\S2NB-Render.MP4"
bpy.ops.wm.save_as_mainfile(filepath=r"C:\Users\UsEr\Downloads\S2NB_previz.blend")
sc.frame_set(1)

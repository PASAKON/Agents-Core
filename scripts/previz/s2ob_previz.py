# S2OB previz — THE REVERSE. Same scene, same marks, camera moved OUT of
...
dupe    = spawn_char(col, "DUPE",      (-2.30, -17.20), h=1.80, prefix="s2ob")
valder  = spawn_char(col, "VALDER",    ( 0.00,  20.50), h=1.86, prefix="s2ob")
guardL  = spawn_char(col, "GUARD_V1", (-1.10,  21.30), h=1.84, prefix="s2ob")
guardR  = spawn_char(col, "GUARD_V2", ( 1.10,  21.30), h=1.76, prefix="s2ob")
gent    = spawn_char(col, "GENTLEMAN", ( 5.00,  -3.00), h=1.79, prefix="s2ob")
bguard  = spawn_char(col, "BODYGUARD", ( 5.95,  -2.15), h=1.94, prefix="s2ob")
regis   = spawn_char(col, "REGISTRAR", ( 3.05,  -4.35), h=1.80, prefix="s2ob")
# the five hold their marks off-frame, for continuity into S2P
woman   = spawn_char(col, "COLLECTOR_A",     (-1.45, -19.95), h=1.68, prefix="s2ob")
student = spawn_char(col, "STUDENT", ( 0.15, -19.75), h=1.70, prefix="s2ob")
wifeB   = spawn_char(col, "VISITOR_B", ( 1.85, -19.70), h=1.66, prefix="s2ob")
manA    = spawn_char(col, "VISITOR_A", ( 2.60, -19.45), h=1.78, prefix="s2ob")
critic  = spawn_char(col, "CRITIC",  ( 3.55, -19.85), h=1.64, prefix="s2ob")
build_cart(col, (-3.30, -16.60), prefix="s2ob", with_painting=True)
cart = bpy.data.objects.get("s2ob_cart_base")
if cart:
    cart.rotation_euler.z = R(90)
    for o in bpy.data.objects:
        if o.name.startswith("s2ob_cart_") and o is not cart:
            o.parent = cart; o.matrix_parent_inverse = cart.matrix_world.inverted()
    box("s2ob_painting", (-3.30, -16.20, 1.35), (0.80, 0.05, 0.72), (0.45, 0.45, 0.45, 1)).parent = cart
...
for who, ob in (("DUPE", dupe), ("VALDER", valder), ("GUARD_L", guardL),
                ...
                ("REGISTRAR", regis)):
    tag_label(col, who, ob, cam, prefix="s2ob")
...
sc.render.filepath = r"C:\Users\UsEr\Downloads\S2OB-Render.MP4"
bpy.ops.wm.save_as_mainfile(filepath=r"C:\Users\UsEr\Downloads\S2OB_previz.blend")
sc.frame_set(1)

# S2MB previz — THE REVERSE. Same scene, same marks, camera moved OUT of
...
woman   = spawn_char(col, "COLLECTOR_A",     (-1.60, -19.95), h=1.68, prefix="s2mb")
student = spawn_char(col, "STUDENT", ( -0.45, -19.72), h=1.70, prefix="s2mb")
wifeB   = spawn_char(col, "VISITOR_B", ( 0.40, -19.88), h=1.66, prefix="s2mb")
manA    = spawn_char(col, "VISITOR_A", ( 1.05, -19.55), h=1.78, prefix="s2mb")
critic  = spawn_char(col, "CRITIC",  ( 1.80, -19.95), h=1.64, prefix="s2mb")
dupe    = spawn_char(col, "DUPE",      (-2.40, -17.20), h=1.80, prefix="s2mb")
regis   = spawn_char(col, "REGISTRAR", (-5.00,  -8.00), h=1.80, prefix="s2mb")
build_cart(col, (-3.30, -16.60), prefix="s2mb", with_painting=True)
cart = bpy.data.objects.get("s2mb_cart_base")
if cart:
    cart.rotation_euler.z = R(90)
    for o in bpy.data.objects:
        if o.name.startswith("s2mb_cart_") and o is not cart:
            o.parent = cart; o.matrix_parent_inverse = cart.matrix_world.inverted()
    box("s2mb_painting", (-3.30, -16.20, 1.35), (0.80, 0.05, 0.72), (0.45, 0.45, 0.45, 1)).parent = cart

cam_d = bpy.data.cameras.new("CAM_S2MB"); cam_d.lens = 35
cam = bpy.data.objects.new("CAM_S2MB", cam_d); sc.collection.objects.link(cam); sc.camera = cam
...
for who, ob in (("WOMAN", woman), ("STUDENT", student), ("WIFE", wifeB),
    ...
    tag_label(col, who, ob, cam, prefix="s2mb")
...
sc.render.filepath = r"C:\Users\UsEr\Downloads\S2MB-Render.MP4"
bpy.ops.wm.save_as_mainfile(filepath=r"C:\Users\UsEr\Downloads\S2MB_previz.blend")
sc.frame_set(1)

# S2S previz — A HUNDRED MILLION, AND THE SLOWEST ENTRANCE IN THE FILM.
#
...
gent   = spawn_char(col, "GENTLEMAN",  (-2.10, -3.00), h=1.79, prefix="s2s")
bguard = spawn_char(col, "BODYGUARD",  (-3.10, -2.10), h=1.94, prefix="s2s")
madame = spawn_char(col, "PARROT",     ( 2.15, -3.20), h=1.72, prefix="s2s")
valder = spawn_char(col, "VALDER",     ( 0.00, -1.55), h=1.86, prefix="s2s")
regis  = spawn_char(col, "REGISTRAR",  (-2.75, -5.50), h=1.80, prefix="s2s")
watchers = {}
for name, x, y, h in (("GUARD_V1",    -1.70,  0.10, 1.84),
    ...
    watchers[name] = spawn_char(col, name, (x, y), h=h, prefix="s2s")
dupe = spawn_char(col, "DUPE", (-4.30, 1.60), h=1.80, prefix="s2s")
build_cart(col, (-4.95, 2.30), prefix="s2s", with_painting=True)
cart = bpy.data.objects.get("s2s_cart_base")
if cart:
    for o in bpy.data.objects:
        if o.name.startswith("s2s_cart_") and o is not cart:
            o.parent = cart; o.matrix_parent_inverse = cart.matrix_world.inverted()
    box("s2s_painting", (-4.95, 2.70, 1.35), (0.80, 0.05, 0.72), (0.45, 0.45, 0.45, 1)).parent = cart
...
chair = bpy.data.objects.new("s2s_chair_root", None)
sc.collection.objects.link(chair)
chair.location = (0.30, 16.00, 0.0)
seat = box("s2s_chair_seat", (0, 0, 0), (0.62, 0.70, 0.10), (0.10, 0.10, 0.12, 1))
back = box("s2s_chair_back", (0, 0, 0), (0.62, 0.08, 0.55), (0.10, 0.10, 0.12, 1))
whL  = box("s2s_chair_whl",  (0, 0, 0), (0.06, 0.56, 0.56), (0.05, 0.05, 0.05, 1))
whR  = box("s2s_chair_whr",  (0, 0, 0), (0.06, 0.56, 0.56), (0.05, 0.05, 0.05, 1))
...
gran = spawn_char(col, "GRANDMOTHER", (0.30, 16.00), h=1.05, prefix="s2s")
gran.parent = chair
gran.location = (0.0, 0.0, 0.52)                  # sitting on the seat

cam_d = bpy.data.cameras.new("CAM_S2S"); cam_d.lens = 30
cam = bpy.data.objects.new("CAM_S2S", cam_d); sc.collection.objects.link(cam); sc.camera = cam
cam.location = (0.00, -11.40, 1.66)
cam.rotation_euler = (R(88), 0, 0)
for who, ob in (("GENTLEMAN", gent), ("MADAME", madame), ("VALDER", valder),
                ("GRANDMOTHER", gran), ("REGISTRAR", regis), ("DUPE", dupe)):
    tag_label(col, who, ob, cam, prefix="s2s")
...
sc.render.filepath = r"C:\Users\UsEr\Downloads\S2S-Render.MP4"
bpy.ops.wm.save_as_mainfile(filepath=r"C:\Users\UsEr\Downloads\S2S_previz.blend")
sc.frame_set(1)

...
LINE = [
    ...
    ("DUPE",      0.60, -2.90, 1.80),   # ~2.6 m behind the last guard (cart must stay inside the frame edge)
]
people = []
for name, x, dy, h in LINE:
    ob = spawn_char(col, name, (x, BASE + dy), h=h, prefix="s2pt")
    people.append((ob, x, dy))
build_cart(col, (-0.15, BASE - 3.60), prefix="s2pt", with_painting=True)
...
if cart:
    ...
    box("s2pt_painting", (-0.15, BASE - 3.20, 1.35), (0.80, 0.05, 0.72), (0.45, 0.45, 0.45, 1)).parent = cart
...
if cart:
    key(cart, 1,   -0.15, BASE - 3.60)
    key(cart, 480, -0.15, BASE - 3.60 + TRAVEL)

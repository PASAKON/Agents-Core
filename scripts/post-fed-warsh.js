#!/usr/bin/env node
/**
 * One-off publisher: FED / Kevin Warsh news poster → MoonieX TradeTech FB.
 * Clone of claudeflow/scripts/post-musk-poster.js pattern (single account,
 * fixed externalId = idempotent, immediate post + poll status='processed').
 *
 * DRY RUN by default (prints plan + caption, touches nothing).
 * Pass --go to upload image + create the post NOW.
 * Re-running is safe: same external_id → PostForMe returns the existing post.
 *
 * Prepared 2026-06-18 by CMO. DO NOT --go without CEO confirmation.
 *
 *   node scripts/post-fed-warsh.js          # dry run — review plan
 *   node scripts/post-fed-warsh.js --go     # live — upload + post
 */
'use strict';
...
const POSTER = AG + '/output/mooniex-posters/fed-warsh/fed-warsh.png';
const BUCKET_NAME = 'fed-warsh-20260618.png';
const EXTERNAL_ID = 'mooniex-fed-warsh-20260618';
const LINK = 'https://mooniex.com/tools?utm_source=fb&utm_campaign=fed_warsh';

const CAPTION = [
  '🔔 คืนนี้ตี 1 (18 มิ.ย. 01:00 น.) จับตาผลประชุมเฟด FOMC',
  'ครั้งแรกของประธานคนใหม่ Kevin Warsh',
  '',
  'ดอกเบี้ยออกทางไหน ตลาดทั้งคืนวิ่งตามทันที 👇',
  '🟢 ลด = ตลาดพุ่งแรง',
  '🟡 คง = ตลาดอาจนิ่ง',
  '🔴 ขึ้น = ตลาดพังแน่!',
  '',
...
(async () => {
  console.log('=== FED/Warsh poster → MoonieX TradeTech — ' + (GO ? '*** LIVE (--go) ***' : 'DRY RUN (no --go)') + ' ===');
  ...
  // 2) create post (1 account, idempotent externalId)
  ...
  // 3) poll until processed (max ~120s)

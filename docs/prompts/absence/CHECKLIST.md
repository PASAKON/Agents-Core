...
| S4 | **$2,000,000** — wall POV. 8 people now, ordinary clothes. | 11s (spec said 15s — see duration-bug note) | 🔴 **take 1 FIRED, rendering** — asset `6aa5bf40-11a3-43e3-bce7-bfcc1ec518ce`. Ref swapped from `loc_wall_pov_b` (dead) to `loc_wall_pov_c` per CTO's live instruction. 9/9 mentions clean, Unlimited $0, 720p, 16:9. Not yet collected |
...
| S7 | **Valder arrives** and greets the gentleman in the white suit. | 20s | 🔴 **blocked, fixing (3rd attempt)** — `char_gentleman` also Face/IP-dead (same terminal message as `loc_wall_pov_b`, 3 faces baked into one plate). `96d067d8` = **superseded**, fired without full CEO spec. `87afaa67` = **superseded**, gold-capped teeth present under zoom but mouth too closed to read clearly in the scripted "smiles, two gold teeth flash" beat (CTO caught this, correctly — S7's beat needs it unmistakable). **Final: `5e5c2eaf-5dfd-419d-bad0-912e63d20771`** — identical full CEO spec, panel 2 now explicitly mouth-open-enough-to-show-teeth. Not yet visually re-verified — landing. Will re-point the existing `char_gentleman` Element to this one once confirmed | task-a13469e9 |

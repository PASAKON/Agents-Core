...
- **S7's "Rights verification required" card is OFF LIMITS — do not click the banner or investigate further without explicit clearance.** CEO approved that click once before for a different clip; consent does not carry over. CTO is asking the CEO directly. The card's only other control is one unlabeled icon-only button with no aria-label/tooltip — deliberately NOT clicked, since its function can't be confirmed without risking the same flow. S7 stays uncollected until cleared.

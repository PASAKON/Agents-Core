...
| S11 | **$100,000,000** — wall POV. Press, live broadcast, helicopter. | 15s | 🔴 **also needs char_press** — same terminal block as S16, plus `char_gentleman` → `char_gentleman_c` swap |
| S12 | **The grandmother** wheels in and takes it. The room freezes. | 25s | 🔴 **blocked on char_press** — same terminal Face/IP failure as S16/S11 (char_press chip shows no-entry icon). Still needs `char_gentleman` → `char_gentleman_c` swap too before firing. Plate `5dd23a87` (character ref) wheels pass all 4 panels · ⚠️ gloves came out black, CTO: skip reshoot |
...
| S16 | **They saw the wall out of the building.** | — | 🔴 **BLOCKED, not fired** — `char_press` is now terminally Face/IP-dead (no-entry icon, confirmed via re-click attempt, no retry option — same failure mode as `loc_wall_pov_b`/`char_gentleman`). S16's ref list needs char_press. Composer prompt staged and clean otherwise (loc_hall_big_d, char_valder, char_guard_valder_six, char_cleaner_c). **Needs a solo/no-crowd replacement plate for the press pair, same fix pattern as the location plates: fewer faces baked into one reference.** |

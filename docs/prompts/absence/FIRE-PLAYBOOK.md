...
- WINDOW WIDTH TRAP (2026-09-07 17:35): a Chrome window that is 1440 px wide LOOKS fine but the extension side panel eats ~300 px, so window.innerWidth lands near 1140 and Higgsfield silently switches to its mobile layout — Generate DISABLED, freeGens undefined, and it looks exactly like a stuck previz check. Measured 17:40: the S2R-JC tab read innerWidth 1440 with the window at 1440, so on this Mac the side panel does NOT shrink innerWidth and 1440 is safe; the trap only bites when innerWidth itself is under 1280. The CTO's OS guard re-widens under 2000 px anyway. Operator rule: a DISABLED Generate → read window.innerWidth FIRST; under 1280 → wait 60 s for the guard, re-read, then continue.

# PLATE · loc_wall_pov_e — THE CANON CRACK

Source still: `docs/prompts/absence/wall-pov-e-source.png` · Element `@project_absence_loc_wall_pov_e`
Bound by every wall-POV sheet (S2H / S2I / S2K / S2L / S2M / S2N and their -B reverses).
The picture: a normal view down the gallery, and ONE small black crack floating at the
very front of the frame, over the far red door. The wall itself is invisible.

## CEO, verbatim, 2026-09-06 morning

> มันไม่ใช่ดาว มันคือรอยร้าวที่เหมือนดาวนั้นแหละ รอยแตกของกำแพง สีดำ
> ห้ามเปลี่ยนรอยนี้เด็ดขาด และรอยนี้ลอยอยู่จุดหน้าสุดของหน้าจอ
> แต่เท่าที่ดูคุณเรียงลำดับถูกแล้ว

Earlier, in `PLATE-loc_wall_pov_f.md`: "The real crack is tiny… size of a hand… A big
crack kills the film."

## In prompt terms

1. **A CRACK, not a star.** Write "a black crack, star-shaped". Never "a star" as the noun.
2. **Solid BLACK.** Not grey, not pale, not glowing.
3. **NEVER CHANGED.** Same shape, colour and size in every scene, copied from the plate.
4. **SMALL.** About a hand across. A big crack kills the film.
5. **FRONTMOST.** It floats at the very front of the frame, in the plane where the wall was;
   every person and every part of the room is behind it. No hole, no edges, no black
   surround — the wall is not there. (The layering in S2K take 1 was already right — CEO.)
6. **WEIGHT BEFORE SIZE — added 2026-09-08 by the CTO, from two measured failures.**
   Every line of the crack is SOLID and carries the weight of ink. There are no
   hairlines, no fine threads, no wisps, and nothing tapers away to a point outside the
   star. Say this BEFORE saying how small it is, because "small" and "thin" are not the
   same instruction and the model treats them as one.

   Why this rule exists. The sheets said "SAME SHAPE — thin lines meeting at one small
   dark point — and SAME SOLID BLACK", putting "thin" and "solid black" in the same
   breath, and thin won twice:
   - S15a-2 take 1 (2026-09-08, 52 credits): fine GREY hairlines with legs radiating
     from a dark dot. At the rendered scale it reads as a mosquito on the plaster.
     That sheet had also merely NAMED the mark in four words, which is the other half
     of the cause — see rule 7.
   - S2G take 2 (2026-09-08, free lane), the insert whose entire subject is the mark
     and which carries the full canon: a correct solid dark star CORE, but three long
     TAN tendrils trailing up-right, left and down, each a hairline and none of them
     black. Predicted in the A/B ledger before the clip landed, then confirmed.
   The star core is not the problem in either take. The tendrils are. Ban them by
   naming what the lines ARE — short, solid, ink-weight, ending where they end.
8. **THE MARK MUST BE SMALL IN FRAME, AND THERE IS A NUMBER FOR IT — measured
   2026-09-08.** The bigger the mark is allowed to be in the picture, the thinner the
   model draws it: at close range it stops drawing a graphic and starts drawing
   realistic plaster fracture, which is fine hairlines by nature. Two takes, same
   canon wording, opposite results:
   - S15a-2 take 2, PASSED: mark 96 x 125 px on a 1280 frame, 7.5% of frame width,
     and the black FILLS 0.12 of its own bounding box — a bold compact shape.
   - S2G take 4, FAILED: mark 170 x 154 px, 13.3% of frame width, and the black fills
     only 0.037 of its bounding box — three times sparser, a spidery web.
   So the acceptance test is measurable and belongs in every REVIEW ORDER that carries
   the mark: **bounding box under about 8% of frame width, fill ratio at or above
   0.10.** If a shot wants the mark bigger than that, move the camera back instead of
   adding adjectives — this is the same lesson SC5's sky taught, that framing beats
   description when a picture is fighting the words.

7. **THE CANON TRAVELS WITH THE MARK.** Any sheet in which the mark is visible carries
   this whole description, even when the mark is background and the scene is about
   people. A sheet that only names it ("the small black mark on the axis") gets whatever
   the model imagines a small black mark to be, and what it imagines is an insect.

## What was wrong before today — all mine, none of it from a plate

- "camera INSIDE the broken wall, looking out through the hole", "the jagged edges of the
  hole… a hard black silhouette", "the ragged black opening frames the picture", "torn hole
  with ragged plaster". A big-hole construct, sitting in the same paste block as the
  "size of a hand" paragraph. Two descriptions of one object: the model rendered the more
  vivid one (S2K take 1 — crack ~5× too big, arms across faces).
- Calling it "a star". The CEO: it is not a star.

**Rule: a prompt describes the mark ONCE.** Fixed in S2K / S2M / S2N first (they fire next);
S2H, S2I, S2L, S2L-B, S2M-B, S2N-B still carry the old prose in their shot lines and get the
same edit one at a time before each fires. S2G's previz still draws a crude star — remove
before S2G fires.

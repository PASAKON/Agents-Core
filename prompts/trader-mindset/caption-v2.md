...
> **DRAFT v2 — NOT ACTIVE.** Generated by `scripts/tm_prompt_evolve.py` from v1 on 2026-06-10.
> Folded (reflected into this prompt): tone×1. Promoted to validator (NOT in prompt): too_long×2.
> Activate ONLY after `scripts/tm_prompt_eval.py` reports a win vs v1, then flip `registry.active.caption`.
...
บทเรียนที่ห้ามพลาดซ้ำ (อัปเดตอัตโนมัติจาก reject รอบก่อน — tm_prompt_evolve):
- [2026-06-10] โทนแข็งไป ไม่เหมือนรุ่นพี่คุยกับรุ่นน้อง อยากให้เป็นกันเองอบอุ่นกว่านี้
```

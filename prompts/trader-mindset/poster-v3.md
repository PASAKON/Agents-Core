# TraderMindset — Poster prompt (v3)
...
```text
{{preamble}} {{scene}} {{style}} '{{hero_word}}' fills the upper-left with bold textured strokes. Render ONLY that single Thai word as art -- do NOT draw any other sentence, sub-headline or small text anywhere. Let the cinematic trading-floor scene FILL THE ENTIRE FRAME edge-to-edge (full-bleed) like a real photograph — NO flat color band, navy strip or empty box anywhere; the lower area stays part of the real scene, only naturally darker so a caption can sit on it later. Render the hero word accurately and legibly.
```

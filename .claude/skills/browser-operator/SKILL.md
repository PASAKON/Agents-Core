...
- 2026-09-24 [MISSING] ChatGPT image runner on winbox — first full live batch after the reinstall: `tools/chatgpt_images.py` against the dedicated 9224 profile (`C:\Users\passg\.chatgpt-automation\chrome-profile`, pwvenv Playwright 1.47, launched from ssh with plain `ssh winbox "<.cmd>"`, CDP reaches the desktop Chrome from session 0) made 8/8 images, exit 0, zero model turns. `ATTACH:` (marked UNVERIFIED in task-854cb512) now works live: 3 of the 8 attached a repo JPEG and the output followed it (same species for the villagers, same sky/moon for the sea and colour script). Trap: `run.log` prints one line per FINISHED image, so the first ~5 min look like a hang; read progress from `curl http://127.0.0.1:9224/json/list` (the tab title becomes the image's subject) instead of killing it · evidence: docs/reports/ilag-trailer-round4-20260924/ledger.json · status: pending
- 2026-09-25 [MISSING] ChatGPT image runner — `tools/chatgpt_images.py` has NO lock on the CDP port, and two sessions now drive the same winbox 9224 Chrome: task-4f4d4bd0 (H3 A/B dancer refs, finished 04:07) and cto-e1e3d3ef's ILAG rounds 4-5 (03:3x and 04:1x). They missed each other only because the H3 worker checked `tasklist` for python.exe first. Until the tool takes a lock (e.g. a lock file next to the ledger or a pc-lease row keyed on the CDP port), check `tasklist /fi "imagename eq python.exe"` and `/json/list` before every run · evidence: docs/reports/h3-abtest-character-20260925/REPORT.md lines 44-47, docs/reports/ilag-trailer-round5-20260925/ledger.json · status: pending
- 2026-09-25 [MISSING] headless runners on winbox — backgrounding a long job from ssh orphans it: both `start /B` and a detached PowerShell `Start-Process` died the moment the ssh session closed (0-byte logs, no error). What worked: run the command SYNCHRONOUSLY over ssh and background it on the Mac side (the session's own Bash `run_in_background: true`) · evidence: task-4f4d4bd0 (two silent orphans, then one clean run, ledger 3/3 done, exit 0) · status: pending
- 2026-09-25 [MISSING] Facebook uploads — the route that needs no human: a dedicated Chrome on a CDP port signed in via the login relay (mac:9230) + Playwright `set_input_files` on the composer's file input (any size, no native dialog) — `tools/fb_reel_post.py`. Two traps in it: the wizard's breadcrumb tab carries the same "แชร์" text as the footer Share button, so a text selector "published" twice while creating nothing (verify in Business Suite → Content, never trust the script's own log); and a CDP Chrome with 0 tabs refuses `connect_over_cdp` ("Browser context management is not supported") — open one with `curl -X PUT :PORT/json/new?about:blank` first · evidence: task-c6bd5ba6, reel 4560927164226012 · status: pending
- 2026-09-25 [COSTLY] §ChatGPT image runner — tools/chatgpt_images.py failed both images of ILAG round 12 twice in a row with `send did not register: no new user turn, no URL change`, about 20 minutes after round 11 (same runner, same kind of attached picture) worked; the page was not inspected, so the cause is unknown; the film fell back to describing the object in words · evidence: C:/mooniex/ilag-runner/out/run12.log, docs/ops/briefs/ilag-trailer-round12.md · status: pending
- 2026-09-26 [COSTLY] network logging — a runner that saved every response BODY into its ledger captured a Google OAuth access token (`ya29…`) from Flow Music's `/__api/auth/google/save`, and part of it printed into the worker's transcript. Log path + status only; read a body only for the one call you need and extract the one field; redact `ya29.`/`eyJ…`/`*_token` before anything is written (tools/flow_music.py `_SECRET_RE`) · evidence: Flow Music run 2026-09-26, fix 35b902a5 · status: pending
- 2026-09-26 [MISSING] §Leaving a replay script behind — throwaway Playwright exploration scripts count too: every script that opens a page on a live CDP Chrome opens it inside `try` and closes it in `finally`, and the run keeps one reused tab. ~35 scratch scripts with a linear `new_page()` … `page.close()` leaked tabs whenever one died on a TimeoutError (3 confirmed); 10 extra tabs were open when the CEO saw it ("Worker วนลูป เปิด tab ไม่หยุด") · evidence: task-cfdc75a8 REPORT.md §Tab leak · status: pending
- 2026-09-26 [COSTLY] §cost discipline — the screenshot budget does not cover tab count or exploration time: a search for one control across 5 Facebook/Business Suite surfaces ran unbounded until the CTO stopped it. Prevented by: a hard time box on exploration (20 min) plus a live tab-count check (CDP `/json/list`) before opening a new page; when the box ends, report the surfaces tried instead of trying a sixth · evidence: task-cfdc75a8, CTO closed 10 tabs · status: pending

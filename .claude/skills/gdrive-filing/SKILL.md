...
├── --help/                       SEEN 2026-09-24 17:47 (winbox rclone listing), EMPTY, not in this
│                                 tree before — looks like an rclone typo (`rclone mkdir gdrive:--help`)
│                                 during the re-consent. id 1aTLoqbTA4Ed3JECBKZ1QML-3fIim_LQr. Rule 2:
│                                 trash only on the CEO's explicit yes; re-check emptiness first.
├── Archive/                      SEEN 2026-09-24, not created by this session and NOT DEFINED (Rule 7:
│   ├── Agents-output/            ask the CEO / the Mac CTO what it is before filing into it). ids
│   └── Backups/                  Archive 1uQo4YCdxVlYLFjD-6LQIBcNAVMoyXdC_ · Agents-output
...
│       └── Sorry, Sir (The Valder Collection)/   the second ILAG film, in
...
│   │   │                         (CEO 2026-09-24). Packing: one `<YYYY-MM-DD>.tar.gz` per slug per
...
│   ├── Claude-Transcripts/       PRE-EXISTING, seen 2026-09-24 (id 1FqUUq9CdwL7AvMnPzlui3kgUtKTMU4ZP):
│   │                             the Mac's `prune_transcripts.py --archive` output before the MoonieX HQ
│   │                             family existed — 112 `<uuid>.tar.gz` in 40 Mac project-slug folders,
│   │                             1.36 GB (34 files / 313 MB under `-Users-gob-Projects-Agents`, the rest
│   │                             one per worker worktree). PROPOSED, not done: move server-side into
│   │                             `MoonieX HQ/Claude-Transcripts/mac/` once the CEO says yes (Rule 1).
...
│   │                             its Element/ once the CEO approves that folder.

...
│       └── StoryBoard (Doc)
│   └── เรื่องตาชั่งของเสี่ย/         NEW 2026-09-25 (CEO "Ok go"). The second story, repo name
│       │                      `taachang`. Same layout as the first story.
│       ├── logs.txt           append-only, 9-field contract; seeded with the 64 files below
│       ├── All Scene/ACT1/    every take: shot-NN.mp4 (take 1), shot-NN-take2.mp4 (QC re-shoot),
│       │                      shot-NN-test360p.mp4 (the 360p emotion test)
│       ├── All Scene/ACT2/    NEW 2026-09-26 (act pattern, pre-approved). S24–53 takes, shot-32/37/51
│       │                      -take2.mp4 (QC re-shoots, in the cut), ACT2-review-540p.mp4 (34 files)
│       ├── All Scene/ACT3/    NEW 2026-09-26 (act pattern). S54–75 takes, shot-70-take2, shot-71-take2
│       │                      (+ -fixed: caption erased), shot-75-muted, ACT3-review-540p.mp4 (27 files)
│       ├── Element/           → Character/ (18 state plates, `<who>__<state>.png`), Location/ (6),
│       │                      Prop/ (11). The CEO-approved ChatGPT plates only (the rejected needle
│       │                      scale and the teak house stay in Work/)
│       ├── Soundtrack/
│       └── Final Draft/
...
│   ├── Mac-Reinstall-2026-09-25-iCloudLeftovers-p01…p08.tar (+ .manifest.json) + `…-p09-f786.mov`   NEW 2026-09-25,
│   │                             CEO in chat ("สำรองด้วย ระหว่างสำรองลบ ข้อมูลที่ยืนยันได้ว่าสำรองแล้ว"). The Mac's orphaned
│   │                             iCloud Drive store (CloudDocs/session/i) minus what was already on Drive by md5. Table row below.
...
| `FB: ละครสั้นคุณธรรม/เรื่องตาชั่งของเสี่ย` | `1GBZivD3J3kgpBYiNuOe8TRrWWjOeh2sn` | The second story (repo name `taachang`), created 2026-09-25 on the CEO's "Ok go" after being asked "สร้างตาม pattern เรื่องแรก". Children: `All Scene` `1EY2525NRGP6wxR0Ed2TUGS9K5mQRn1vh` (→ `ACT1` `1__mUC5382-Ou0XhAybFotvvL8ID20mD-`, `ACT2` `1_gWS966sPLNx4gZv_tYwFSEEEl2mAMo0` (2026-09-26, 34 files, md5-verified), `ACT3` `1hTiHBCIQ587fNk2obIyBE3_NJVV6bHuR` (2026-09-26, 27 files, md5-verified); `ACT<n>` per act, same pattern), `Element` `1Cuk8_y4UxH3YWXWbSPl3qYOhnXGzJrMR` (→ `Character` `1BhErdoWaOH4siLp9dSdsfxQnggREaYyH`, `Location` `1d6mJDPAD38--zgpqg3kG9Lb3sLSBqcGL`, `Prop` `1_afRsEkgF6awd4y0qPqwGXMEXiPHtiVW`), `Soundtrack` `1mKB8vpyavAklsyee9KJEwAXSrSzdHQ7j`, `Final Draft` `1TCytBfA8DFznVDSZuxLnZYI5g7i_PZxM`, `logs.txt` `1MIB39c1KbpAQr9R25waFwxfF5R2PzbWw`. Plates are named `<who>__<state>` (the state system, CTO_Flow_Omni1.1_Continuity rule 8) and stay on the Mac too (build_shotsheet needs them). Uploaded through winbox rclone by folder id, md5-verified (64/64). |
...
| `BACKUP/Mac-Reinstall-2026-09-25-iCloudLeftovers-*` (BACKUP root) | p01 `12qrIIjmUeIB0n_Lye12h-i51Ba1ePjEi` · p02 `1oWLinIEHnVSQgpv9SSOWzIcdGOO79MzL` · p03 `10JjQvCISeeWuCVmPApOkcPx1MDj_nmer` · p04 `1pJODmO1AlDNde3hyh9zTL1iOHvUZnuKs` · p05 `1xxBUjDbv1pfMZsVAoaspAKYa-HIJ9bTS` · p06 `1EzRRV8RG6J3awW4je2IbAS1vGdqWLxcG` · p07 `1C3CR-tiwU5WnLOz_VKE9723M8MtcmQXw` · p08 `183Oi3Zxfygy8-wsdbz6rjNUmW3iT7xbV` · p09 (raw .mov) `1JLxWnny9i1hZpqyHGjHuoiEkW4M9kPET` | NEW 2026-09-25, CEO in chat ("สำรองด้วย ระหว่างสำรองลบ ข้อมูลที่ยืนยันได้ว่าสำรองแล้ว ออกให้เลยเป็นระยะ ๆ"). The Mac's **orphaned iCloud Drive store** `~/Library/Application Support/CloudDocs/session/i` (1,050 files / 43.3 GB, not in client.db, invisible in Finder). Files whose md5 was already on Drive were not re-uploaded; the other 491 went up as p01–p08 (490 files, 19.65 GiB, tars with `.manifest.json`, size+md5 read back by id) plus **p09 = one 7.0 GB .mov uploaded raw, not tarred** (`Mac-Reinstall-2026-09-25-iCloudLeftovers-p09-f786.mov`, md5 `edbd272a…`; the disk was too full to spool a tar). Proven media went to the Trash and the CEO emptied it; the ~15 **documents were backed up but left on the Mac** for the CEO to review himself. Per-part ids, md5 and file counts in `~/.claude/logs/drive-archive.log`. |

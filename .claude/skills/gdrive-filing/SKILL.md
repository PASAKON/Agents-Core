...
│           ├── Meta (Higgsfield)/ NEW 2026-09-12 — same four festival assets as
│           │                      Sorry, Sir (server-side copy, identical md5)
...
│           │                      ตั้ง folder ไว้ด้วย"). The festival-mandated
│           │                      Higgsfield branding: watermark PNG + packshot
│           │                      ProRes, 16:9 and 21:9. Same four files exist
│           │                      under Do Not Disturb/ — the CEO asked for a copy
│           │                      per film rather than one shared set
...
| `Sorry, Sir/Meta (Higgsfield)` | `1cV5SkpAd5xrargbiyF4wmL2Y_rEoy6CS` | Created 2026-09-12 by rclone on winbox, CEO-approved in chat ("เอา ใส่ Upload Drive ไว้เลยตั้ง folder ไว้ด้วย"). The branding the festival **requires** on the delivered film: `higgsfield_watermark_16_9.png` (3840×2160, `1SCmNdnlTfVMgDt1xxoSx850dk4Zos_wv`), `higgsfield_watermark_21_9.png` (5120×2160, `16FOhA-U33hS_4MhCbuv1pmkWKrlZdy0u`), `higgsfield_packshot_16_9.mov` (ProRes 4444 w/ alpha, 3840×2160, 9.13s @30, `1hlzWomoKIb70EyTXr2k-1yIlhXk_Pm4N`), `higgsfield_packshot_21_9.mov` (3840×1646, `1vmykFwbcl1lxTjlSRt3nI8Txr3yh63WB`). 1.93 GB, uploaded from winbox `Downloads` with `rclone copy --transfers 1 --drive-chunk-size 64M`, verified `rclone check --one-way` → 0 differences / 4 matching. Not footage and not a generation plate, which is why no existing folder fitted. **The 1080p watermarks winbox also holds are deliberately NOT here** — the 4K ones downscale cleanly to the 720p master and there is no reason to ship the smaller pair. |
| `Do Not Disturb/Meta (Higgsfield)` | `1uNBmXIh7RukK4auf2T0juFD_cG9Z9Quq` | Created 2026-09-12, same approval. The identical four files, placed by a **server-side** `rclone copy` from the Sorry, Sir folder (seconds, no re-upload), verified `rclone check` → 0 differences: packshot 16:9 `1jlsBu0gn1O0R9Vi-QtzmcXgpGldO8Jsw`, 21:9 `1zPHdGxOGH370nBoOkPutIY67mA6ca80l`, watermark 16:9 `1uJCZ3lMvaSBV-qtFgdUpXXO4xhFpeOVR`, 21:9 `1v2Ga4qJIK_MbVAyFqOIMojW9mAPOpP4h`. The CEO asked for a copy per film ("แยกเอาใส่ folder DND และ Folder Sorry sir") rather than one shared set — that costs 1.93 GB twice and is his call, recorded here so nobody "tidies" it into a single copy later. |

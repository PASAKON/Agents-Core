...
- 2026-09-23 [MISSING] §Green — abandoned Work/ folders were on no list; now Green via `workdir.py close --archive` (never a plain delete) · evidence: CEO ruling 2026-09-23 "นับเป็น Green ได้ ถ้าไม่ได้ทำต่อแล้วนานมากๆ หรือฉันลืม" · status: promoted
- 2026-09-23 [MISSING] §contabo — the 2026-09-10 baseline is stale: Contabo reached 17 GB free (56/72 GB). The easy wins ran with the CEO's go (build cache 10.31 GB, journal → 500 MB, pip 539 MB), and `/root/idm-packs` (12.29 GB) turned out to be a byte-for-byte copy of Drive `BACKUP/CookieRun Backup/colab_packs`. Per-file md5 against Drive `md5Checksum` matched 47/47, so it was deleted: 17 → 42 GB free. Check whether a big `/root/*` data folder is a copy of a Drive folder before proposing a tar · evidence: session cto-0e8d80b8, `~/.claude/logs/drive-archive.log` 2026-09-23T13:56Z, docs/ops/contabo-hq-migration-plan-2026-09-23.md · status: pending
- 2026-09-24 [MISSING] §mac — the hidden big consumer is live Chrome, not our folders: every running Chrome (the CEO's own, each automation profile on a CDP port) holds a ~1.4 GB `code_sign_clone` under `/private/var/folders/*/*/X/com.google.Chrome.code_sign_clone/`. Three at once = 4.2 GB, invisible to a du of Work/, worktrees and caches. Never delete a clone of a running Chrome. Find which ports are live (`pgrep -fl 'Google Chrome.app/Contents/MacOS/Google Chrome' | grep -o 'remote-debugging-port=[0-9]*'`) and ask each owner to quit an idle one. Quitting the 9223 Flow Chrome freed 1.43 GB within seconds · evidence: CTO 3d312dd6 00:40-00:55, free 3.5 → 7.5 GB after 9223 + 9226 quit · status: pending
- 2026-09-24 [MISSING] §winbox — git "unpushed/dirty" counts on winbox lie until you `git fetch`: `C:\mooniex\agents` read "112 unpushed / 3143 dirty"; after fetch it was 0 ahead, and the dirt was 3,134 tracked files DELETED from the tree + `.venv`. The 26.6 GB `worker-worktrees\room-*\exec-*` are 30 plain git worktrees of that repo (`worker/*` branches 47–71 ahead, 3 dirty files) — regenerable; one thin `git bundle --branches --not origin/main` (2.1 MB) plus patches/untracked holds everything unique. Before backing up a clone: fetch, `--ignore-cr-at-eol`, split modified from deleted · evidence: session cto-46fb0d60, code-agents.tar 1jjw_lQA…, restore-tested · status: pending
- 2026-09-24 [COSTLY] §winbox — two traps in remote survey scripts ate a round each: PowerShell variables are case-insensitive (`foreach($d …)` silently overwrote `$D`, the Downloads path), and a Python `print` to the ssh console (cp1252) crashes on Thai file names. scp a `.ps1` and run it with `-File`, and have Python write UTF-8 files / encode with `backslashreplace` · evidence: session cto-46fb0d60, scripts/stream_backup_to_drive.py log() · status: pending
- 2026-09-24 [COSTLY] §mac — backing up before a wipe, a repo with no `origin` looked remote-less (ClaudeSign's remotes are `mooniex` + `upstream`), so a full `git bundle --all` (3.4 GB) was staged on a Mac with 7 GB free and left it at 4.2 GB. Detect "not on any remote" with `rev-list --branches --not --remotes` and `git remote` (any name), never `remote get-url origin`; the thin bundle was 1.1 MB · evidence: session cto-46fb0d60, Mac-Reinstall-2026-09-24-MoonieXHQ-code.tar · status: pending
- 2026-09-24 [COSTLY] §mac — bsdtar applies `-C` positionally: `tar -cf - -C ~ -T list -C /tmp extras` silently packed ONLY `extras/` (4 entries instead of 219) and still exited 0 with a matching sha256. Put every path in the one `-T` list (absolute for the odd one) and count the entries on the far side, because a matching hash only proves the transfer · evidence: contabo mac-home-extras-20260924.tar first attempt · status: pending
- 2026-09-24 [MISSING] §mac — before a Mac wipe, `~/Pictures/Photos Library.photoslibrary` is on no list and was the biggest thing on the disk (51 GB, 8,654 items). iCloud Photos was OFF (no CLOUDPHOTOS service in MobileMeAccounts.plist; iCloud account signed out), so the Mac held the only copy it had, and the CEO did not know it was there ("I don't even see it"). Check it every time: size, `ZASSET` count, iCloud Photos state; back it up in ≤3 GB parts when disk is tight · evidence: session cto-46fb0d60, 20 Photos tars 52.76 GB · status: pending
- 2026-09-24 [MISSING] §winbox (references/winbox.md) — a naive directory walk on Windows LOOPS through the legacy junctions `%USERPROFILE%\Local Settings` → `AppData\Local` and `AppData\Local\Application Data` → `AppData\Local` (Python < 3.12 `os.walk(followlinks=False)` does not treat a junction as a link): the first `machine_doctor.py --machine winbox check` measured AppData at 98.7 GB when its real contents were ~10 GB (Chrome 5.2, Microsoft 2.5, Programs 1.7). Any size scan on winbox must prune reparse points (`st_file_attributes & 0x400`) — fixed in the doctor (`_is_link`/`_prune_links`, commit 1c9a525a); `du`-style PowerShell one-liners are safe because `Get-ChildItem -Recurse` skips reparse points by default · evidence: winbox doctor output 2026-09-24 ~18:40 UTC · status: pending
- 2026-09-25 [MISSING] §Green — `~/.npm/_npx` (the npx package cache, 1.1 GB on the Mac) is regenerable (npx re-downloads on demand) but the Green list names only "npm"; name `_npx` explicitly. Same day: Homebrew's download cache alone was 3.1 GB; `brew cleanup --prune=all -s` + removing `_npx` took the Mac from 3.9 to 8.1 GB free with no ledger line needed · evidence: CTO 91a17eb2, smoke spawn task-43b6514d queued for disk · status: pending
- 2026-09-25 [MISSING] §mac — `~/Library/Application Support/CloudDocs/session/i` can be an ORPHANED iCloud Drive store: 1,050 files / 43.3 GB that client.db no longer references, invisible in Finder and not counted in Settings' iCloud storage, still on disk after iCloud Drive stopped syncing them. It is on no list. Before a wipe: md5 every file against Drive, back up what is not there, send proven media to the Trash, hand documents to the CEO · evidence: session cto-46fb0d60, Drive `Mac-Reinstall-2026-09-25-iCloudLeftovers-p01…p09` · status: pending
- 2026-09-25 [MISSING] §mac — moving to `~/.Trash` frees NOTHING on APFS (a same-volume rename): with 76 GB of verified-backed-up data sitting in the Trash the Mac fell to 1.8 GB free and a peer's film shoot paused; free space came back (73 GB) only when the CEO emptied it. Emptying is permanent, so it is the CEO's; state the exact size and that it is the only lever the moment anything big goes to the Trash · evidence: df 22:39 1.8 GB → 22:49 73 GB, session cto-46fb0d60 · status: pending

...
- 2026-09-26 [WRONG] §After posting — task-cfdc75a8's report said a Reel post has no delete control (5 surfaces checked). Measured by the CTO: open `https://www.facebook.com/reel/<id>`, click the FIRST visible `[aria-label="เมนู"]` (index 0). Its menu lists "ลบ". The LAST matching button belongs to the preloaded next card, and its menu is the viewer menu (สนใจ / ไม่สนใจ / รายงาน). The `/videos/<id>` "more options" menu has only save / copy link. On the same page the comment is `article` "ความคิดเห็นจาก ละครสั้นคุณธรรม by ILAG Studio", so the first comment was posted as the Page, and exit 7 was a lookup miss, not a wrong author. The click on "ลบ" is not measured yet: the auto-mode classifier refused it pending an explicit CEO approval · evidence: TEST post 4116998775270501, CTO probe 2026-09-26 19:55 (scratch fb_steps.py) · status: pending

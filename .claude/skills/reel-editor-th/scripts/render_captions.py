...
def draw_punch(img, t):
    ...
    if not (st_ <= t < en) or not txt: return
    d = ImageDraw.Draw(img)
    age=t-st_
    s=ease_out_back(clamp(age/0.3))
    f, _ = fit_font(txt, 980, 92, "bold")
...
def draw_counter(img, t):
    """Optional small top-right tag from timeline.COUNTERS = [(start,end,label),...].
    Reuses the save_sticker slot/position (safe as long as windows don't overlap)."""
    for s0, s1, label in getattr(T, "COUNTERS", []):
        if s0 <= t < s1:
            d = ImageDraw.Draw(img)
            f = font(34, "bold")
            tw = int(f.getlength(label)); asc, desc = f.getmetrics()
            x = T.W - tw - 70; y = 250
            rounded(d, [x-40, y-12, x+tw+24, y+asc+desc+8], 26, fill=GREEN)
            d.text((x-16, y), label, font=f, fill=INK)
            break

_IMG_CACHE = {}
def _cover_crop(src):
    """Cover-crop a PIL RGB image to the canvas + a dark/moody grade (matches
    this episode's own brief spec: contrast+, shadows-, saturation-)."""
    sw, sh = src.size
    scale = T.H / sh
    nw = int(sw * scale)
    src = src.resize((nw, T.H))
    x0 = (nw - T.W) // 2
    src = src.crop((x0, 0, x0 + T.W, T.H))
...
def _load_photo(path):
    ...
    if im is None:
        im = _cover_crop(Image.open(path).convert("RGB"))
        _IMG_CACHE[path] = im
    return im

def _load_video_frame(folder, fps, local_t):
    idx = int(local_t * fps)
    key = (folder, idx)
    ...
    if im is None:
        ...
        im = _IMG_CACHE.get(key)
        if im is None:
            im = Image.open(os.path.join(folder, files[idx])).convert("RGB")
...
def draw_kinetic(img, local_t, dur, lines, accent_idx=()):
    """Full-frame kinetic-typography cutaway: big bold word(s) pop in on a
    dark card, matching lines' words in accent_idx are colored GREEN/red."""
    d = ImageDraw.Draw(img)
    d.rectangle([0, 0, T.W, T.H], fill=(12, 14, 18, 255))
    pop = ease_out_back(clamp(local_t / 0.28))
    f = font(112, "bold")
    lh = 128
    total_h = lh * len(lines)
    y0 = T.H // 2 - total_h // 2
    for i, ln in enumerate(lines):
        col = (255, 92, 92, 255) if i in accent_idx else WHITE
        fscale = 0.85 + 0.15 * pop
        lf, _ = fit_font(ln, 980, int(112 * fscale), "bold")
        draw_text_outline(img, (T.W // 2, y0 + i * lh + lh // 2), ln, lf, col,
                           (0, 0, 0, 255), 10, anchor="mm", shadow=(0, 8, (0, 0, 0, 170)))

def draw_traps_card(img, local_t, dur):
    """Full-frame version of the 3-trap checklist (safe here -- no face to avoid)."""
    d = ImageDraw.Draw(img)
    d.rectangle([0, 0, T.W, T.H], fill=(12, 14, 18, 255))
    box = [140, 560, 940, 1360]
    rounded(d, box, 34, fill=(255, 255, 255, 255))
    d.rounded_rectangle([140, 560, 940, 636], radius=34, fill=(16, 124, 16, 255))
    d.rectangle([140, 600, 940, 638], fill=(16, 124, 16, 255))
    d.text((172, 578), "3 กับดักจิตวิทยา", font=font(38, "semi"), fill=WHITE)
    rows = ["Revenge Trade", "Overconfidence", "ตัด SL ไม่ได้"]
    nshow = int(clamp(local_t / max(0.1, dur - 0.3) * 1.2) * len(rows) + 0.001)
    yy = 660
    for i, label in enumerate(rows):
        d.rectangle([164, yy, 916, yy + 96], outline=(220, 224, 220, 255), width=2)
        d.text((188, yy + 24), label, font=font(40, "medium"), fill=(30, 36, 30, 255))
        if i < nshow:
            d.line([820, yy + 48, 848, yy + 74], fill=(16, 150, 60, 255), width=6)
            d.line([848, yy + 74, 892, yy + 22], fill=(16, 150, 60, 255), width=6)
        yy += 110

def _current_cutaway_kind(t):
    for s0, s1, kind, _ in getattr(T, "CUTAWAYS", []):
        if s0 <= t < s1:
            return kind
    return None

def draw_cutaway(img, t):
    """Full-frame opaque cutaway from timeline.CUTAWAYS =
    [(start,end,kind,payload),...] where kind is "photo" (payload=path),
    "video" (payload=(frame_folder,fps)), "kinetic" (payload=(lines,accent_idx)),
    or "card" (payload=None). Hard cut in/out (no fade) -- this clip's own
    brief says "Transition: Hard cut every scene, no fade"."""
    for s0, s1, kind, payload in getattr(T, "CUTAWAYS", []):
        if s0 <= t < s1:
            local_t = t - s0
            dur = s1 - s0
            if kind == "photo":
                img.paste(_load_photo(payload), (0, 0))
            elif kind == "video":
                folder, fps = payload
                img.paste(_load_video_frame(folder, fps, local_t), (0, 0))
            elif kind == "kinetic":
                lines, accent_idx = payload
                draw_kinetic(img, local_t, dur, lines, accent_idx)
            elif kind == "card":
                draw_traps_card(img, local_t, dur)
            return

def build_frame_scene(t, style_name):
    """Everything EXCEPT the word-by-word caption: cutaways, hook, punch,
    counter tag, CTA. Rendered/composited first (the "picture lock")."""
...
def build_frame_subs(t, style_name):
    """ONLY the word-by-word caption -- composited last, on top of the
    finished scene, so it never bakes in whatever's behind it."""
    st = STYLES[style_name]
    img = Image.new("RGBA",(T.W,T.H),(0,0,0,0))
    # kinetic/card cutaways already ARE the caption (big styled text); photo/video
    # cutaways are pure B-roll so the normal word caption keeps running under them.
    if t >= 4.6 and _current_cutaway_kind(t) not in ("kinetic", "card"):
        draw_caption(img, t, st)
    return img

def build_frame(t, style_name):
    """Combined preview (scene + subs); used by the __main__ test harness only."""
    img = build_frame_scene(t, style_name)
    img.alpha_composite(build_frame_subs(t, style_name))
    return img

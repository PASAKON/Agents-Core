...
# PLACEHOLDER CONTENT -- this is the SHARED master copy of the skill. Do NOT
# edit this file in place for a specific clip (two video_editor tasks running
# in parallel both mutate this same file with no lock -- confirmed 2026-08-03:
...
def cover_clean(face_path, out):
    ...
    grad(img,150,235)
    d=ImageDraw.Draw(img)
    tag(d,W//2,220,"[HOOK_TAG]",bg=GREEN,size=52)
    outline_text(d,(W//2,430),"[headline line 1]",fit("[headline line 1]",980,110),WHITE,10)
    ...
    d.text((272,1264),"[checklist title]",font=font(30,"semi"),fill=WHITE)
    yy=1330
    for r,label in enumerate(["[item 1]","[item 2]","[item 3]"]):
        d.rectangle([264,yy,816,yy+62],outline=(220,224,220,255))
        d.text((284,yy+14),label,font=font(28,"medium"),fill=(30,36,30,255))
        d.line([760,yy+31,772,yy+43],fill=(16,150,60,255),width=4); d.line([772,yy+43,790,yy+19],fill=(16,150,60,255),width=4)
        yy+=66

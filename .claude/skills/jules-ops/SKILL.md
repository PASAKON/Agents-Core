...
- 2026-09-23 [WRONG] §2 line 7 — asking Jules to "state which model you are running as" produces a guess, not a fact: PR #163 body said "Model running as: Claude 3.5 Sonnet" (Jules runs Gemini) · evidence: PASAKON/Agents-Core#163 body · fix: never ask a session for its model; the model stays "unknown — not verified" · status: pending
- 2026-09-23 [COSTLY] §4 — first storage PR (#163, ~10 min, 2 files, 8.8 KB, scope clean) passed its own run but failed on the Mac: test spawned bare `python` (FileNotFoundError — no `python` on the Mac's PATH) and `git diff` trusted the user's `diff.renames` (false → a `git mv` flagged). Both only visible by running the verdict locally, as §4 says · evidence: Agents-Core#163, re-dispatched as J3b session 13076616102074107123 · prevented by: brief line 5 names "no `python` on PATH; use sys.executable" for any Python test that spawns a subprocess · status: pending
- 2026-09-23 [MISSING] §3 — `AUTO_CREATE_PR` does not guarantee a PR: J1 (session 16735978756209495482, same tool + payload as J3 which got PR #163) ended COMPLETED with `outputs[].changeSet` only. Recover with `jules_batch.py diff`, `git apply` on a branch, push, and start the fix session from that branch (`--branch`) · evidence: storage1 ledger, branch jules/storage-j1 fc58c49c · status: pending
- 2026-09-23 [MISSING] §3 — a review comment on a Jules PR restarted its COMPLETED session (J3 941641718597106202 flipped back to IN_PROGRESS minutes after the REOPEN comment on #163); `sendMessage` still 404s, but the PR thread is a live channel. Do not post a REOPEN comment AND open a new session for the same fix — two sessions then race on one defect list · evidence: storage1 monitor log · status: pending
- 2026-09-23 [COSTLY] §3/§2 — the comment-restarted session ran WITHOUT the brief's discipline: instead of fixing the four listed defects it pushed `f4a4860f` "fix dependencies (mcp<2, anyio>=4.14.2) to pass CI checks" — a requirements.txt edit that line 3 forbids, the same drift session 1 showed. Review by comment is unsafe; reject by closing the PR (no comment) and re-dispatch with a full 8-line brief · evidence: Agents-Core#163 commit f4a4860f, closed · prevented by: never comment on a Jules PR you are rejecting · status: pending
- 2026-09-23 [WRONG] §2 line 7 — "FACTS, NOT GUESSES" in the brief did not stop fabricated evidence: PR #164's body printed 21 test dots then "1 passed in x.xxs" — a placeholder, not a run. The brief line is necessary, not sufficient; the §4 local verdict run is the only proof · evidence: Agents-Core#164 body · status: pending
- 2026-09-23 [MISSING] §7 — storage1 batch (ADR 0030), 5 sessions on Agents-Core: ≥3 sessions ran concurrently alongside the 18-session A/B batch without queueing; wall 9–13 min each; PR opened by 2 of 4 fresh sessions (J3, J1b yes; J1, J3b diff only); first pass correct 0/2, after one brief-driven fix round 2/2 (+1 CTO safety fix on J1b: NEVER root + scratch); scope discipline held in every brief-driven session (only the named files) and broke only in the comment-restarted one · evidence: state/jules/storage1.jsonl, merges 09f14551 + e1167f1f · status: pending
- 2026-09-23 [MISSING] §3 — update to the PR rate: 3 of 5 fresh sessions ended with `changeSet` only (J1, J3b, J2), 2 opened a PR (J3, J1b). Treat "no PR" as the normal case: recover with the session's `outputs[].changeSet.gitPatch.unidiffPatch`, `git apply --whitespace=fix` on a scratch branch off origin/main, verify on the Mac, push. Every diff so far carried trailing whitespace (21/4/69 lines) · evidence: storage1 J2 5344106204934374537 → 1b96c423 · status: pending

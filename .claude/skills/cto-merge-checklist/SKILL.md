...
- 2026-09-22 [MISSING] §2 — a `os.environ.get(X) or _read_dotenv_var(X)` fallback makes "no key set" tests silently read the real gitignored `.env` and go LIVE; the autouse fixture must monkeypatch the dotenv reader too. Ask on review: does any test reach a paid endpoint if `.env` exists? · evidence: task-a6129a75 — 2 unplanned live Jev calls (~$0.00004) before the fixture fix · status: pending

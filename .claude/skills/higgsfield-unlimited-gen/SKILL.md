...
6. **HARD — Never enter prompt text with a keystroke-simulating "type"
   ...
   **Why hard:** irreversible and money-adjacent. The truncated prompt fires
   a real generation immediately — there is no catching it before Generate,
   only redoing it after, which spends the account's single serial render
   slot on unusable footage and, per Incident 2 above, has already fired a
   real credit charge with zero deliberate click.
